...
def infill_code(example, i, max_to_generate=256, temperature=0.2):
    parts = example.split("<infill>")
    result = infill(parts, max_to_generate=max_to_generate, temperature=temperature)
    print("completed document:")
    print(result["text"])
    base_path = "../baselineResult2/"
    file_path = base_path + str(i) + ".txt"
    infills_file_path = base_path + str(i) + "_infills.txt"
    f = open(file_path, 'w')
    f1 = open(infills_file_path, 'w')
    f.write(result['text'])
    f1.write(" ".join(result['infills']))
    return result["text"]
...
if __name__ == "__main__":
    ...
    base_path = "../baseLine/"
    reulst_path = "../baseLineResult/"
    files = os.listdir(base_path)
    for i in files:
        # print(i)
        print(reulst_path+i)
        if os.path.exists(reulst_path+i):
            print(i)
        else:
